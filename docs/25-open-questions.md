...
3c. **Enlaces de imagen almacenados frente al plazo de 24 h del §2(h).** La Licencia de PI §2(h) permite «guardar un enlace al Contenido Publicitario del Producto durante un plazo máximo de 24 horas». `data/catalog.json` guarda hoy 5 URLs de imagen de `m.media-amazon.com` de forma indefinida (añadidas el 2026-08-31 porque la URL derivada del ASIN devuelve el placeholder 1×1 en esos productos). La URL derivada del ASIN es distinta: se computa en build a partir del ASIN, que sí es almacenable indefinidamente. **Pregunta:** ¿(a) incluimos este punto en la misma solicitud escrita a Amazon del #3b; (b) retiramos `images[]` y aceptamos que esos productos muestren el placeholder propio con iniciales; o (c) los sustituimos por fotografía propia o imagen de prensa del fabricante bajo licencia editorial? Por defecto, mientras no se decida: se mantiene (a) con el riesgo declarado en `docs/24` y la validación automática de `scripts/probe-images.mjs` sobre `images[0]`. Decidir antes de superar los 20 productos con `images[]`.
